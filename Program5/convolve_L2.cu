...
void convolve_h(float *in_image, int width, int height, float *mask, int mask_width, float *out_image)
{
    ...
    for(int k=0;k<mask_width;k++)
    {
        ...
        else //have faith that global accesses cache
        {
...
int main(int argc, char **argv)
{
    ...
    convolve_v<<<dimGrid,dimBlock, sizeof(float)*T*T>>>(d_org_img,width,height,d_gaussian_kernel,k_width,d_temp_horizontal); 
	convolve_h<<<dimGrid,dimBlock, sizeof(float)*T*T>>>(d_temp_horizontal,width,height,d_gaussian_deriv,k_width,d_horizontal_gradient);
    
    //Vertical gradient. horizontal kernel then vertical derivative
    convolve_h<<<dimGrid,dimBlock, sizeof(float)*T*T>>>(d_org_img,width,height,d_gaussian_kernel,k_width,d_temp_vertical); 
	convolve_v<<<dimGrid,dimBlock, sizeof(float)*T*T>>>(d_temp_vertical,width,height,d_gaussian_deriv,k_width,d_vertical_gradient);
