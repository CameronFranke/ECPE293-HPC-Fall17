...
void mag_and_phase(float *horizontal_gradient, float *vertical_gradient, int source_x, int source_y, float **magnitude, float **phase, int start_y, int my_num_rows);

struct thread_data
{
    int group_id, threads_in_group, imageX, imageY, w;
    char type;
    float *image, *temp_image, *horizontal_gradient, *vertical_gradient, *gaussian, *gaussian_d, *magnitude, *phase;
    pthread_barrier_t *barrier_a, *barrier_b, *barrier_all, *barrier_all_done;
};
...
int main(int argc, char *argv[])
{
    ...
    pthread_attr_t attr; 
    pthread_barrier_t barrier_va, barrier_ha, barrier_vb, barrier_hb, barrier_all, barrier_all_done;
	...
	pthread_barrier_init(&barrier_all, NULL, processors);
	pthread_barrier_init(&barrier_all_done, NULL, processors);
	...
	//allocate space for magnitude and phase images 
    ...
    for (int x = 0; x < processors; x++)
    {
        ...
        thread_args[x].magnitude = magnitude;
        thread_args[x].phase = phase;	
        thread_args[x].horizontal_gradient = horizontal_gradient;
        thread_args[x].vertical_gradient = vertical_gradient;
        ...
        thread_args[x].barrier_all = &barrier_all;
        thread_args[x].barrier_all_done = &barrier_all_done;
...
void *create_gradients(void *thread_args)
{
    ...
    float *horizontal_gradient = mydata->horizontal_gradient;
    float *vertical_gradient = mydata->vertical_gradient;
	float *magnitude = mydata->magnitude;
	float *phase = mydata->phase;
	...
	pthread_barrier_t *barrier_all = mydata->barrier_all;
	pthread_barrier_t *barrier_all_done = mydata->barrier_all_done;   	
    ...
    if (type == 'v')
    {
      convolve(source_image, imageX, imageY, gaussian, w, 1, &temp_image, my_start, rows_to_compute); 
      pthread_barrier_wait(barrier_a);      
      convolve(temp_image, imageX, imageY, gaussian_d, 1, w, &vertical_gradient, my_start, rows_to_compute);
      pthread_barrier_wait(barrier_b);
   
      if (group_id == 0)
      {
		char nameh[30] = "Vertical_gradient.pgm";
		write_image_template<float>(nameh, vertical_gradient, imageX, imageY);
      }
    }
    ...
    if (type == 'h')
    {
      ...
      pthread_barrier_wait(barrier_a);
      convolve(temp_image, imageX, imageY, gaussian_d, w, 1, &horizontal_gradient, my_start,rows_to_compute);
      pthread_barrier_wait(barrier_b);
	  ...
	  my_start = my_start + (rows_to_compute/2);

    }
	rows_to_compute = rows_to_compute/2;
	pthread_barrier_wait(barrier_all);  	// wait for all threads from both groups to sync up 
	mag_and_phase(horizontal_gradient,vertical_gradient, imageX, imageY, &magnitude, &phase, my_start,rows_to_compute);
    pthread_barrier_wait(barrier_all_done);
	if (group_id ==0)
	{
		if (type == 'h')
		{
			char name[30] = "Phase.pgm";
			write_image_template<float>(name, phase, imageX, imageY);
		}
		if (type == 'v')
		{
			char name[30] = "Magnitude.pgm";
			write_image_template<float>(name, magnitude, imageX, imageY);
        }
	}
...
void mag_and_phase(float *horizontal_gradient, float *vertical_gradient, int source_x, int source_y, float **magnitude, float **phase, int start_y,     int my_num_rows)
{
    for(int i=start_y; i<(start_y + my_num_rows); i++)
    {   
